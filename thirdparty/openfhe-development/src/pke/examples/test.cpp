...
int main() {
   ...
   */
    ...
    std::vector<double> x1 = {32, 30, 31, 28, 27, 32, 32, 32};
    std::vector<double> x2 = {32, 32, 32, 32, 32, 32, 32, 32};
    ...
    std::vector<std::complex<double>> Result = result->GetCKKSPackedValue();
    std::cout << "ct1 ";
    ...
    result->SetLength(batchSize);
    Result = result->GetCKKSPackedValue();
    std::cout << "ct2 ";
    for (int i = 0; i < int(batchSize); i++)
        std::cout << ", " << Result[i];
    std::cout << std::endl;
    ...
    std::cout << "number of levels remaining before mul: " << multDepth - c1->GetLevel() << std::endl;
    auto cMul_1 = cc->EvalMult(cSub, c1);
    auto cMul_2 = cc->EvalMult(cSub, c2);
    cc->Decrypt(keys.secretKey, cMul_1, &result);
    result->SetLength(batchSize);
    Result = result->GetCKKSPackedValue();
    std::cout << "c1*(c1-c2)= ";
    for (int i = 0; i < int(batchSize); i++)
        std::cout << ", " << Result[i];
    std::cout << std::endl;
    cc->Decrypt(keys.secretKey, cMul_2, &result);
    result->SetLength(batchSize);
    Result = result->GetCKKSPackedValue();
    std::cout << "c2*(c1-c2)= ";
    for (int i = 0; i < int(batchSize); i++)
