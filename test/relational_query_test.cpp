...
double Eval_E2E_q1(int records_num)
{
    ...
    for (size_t i = 0; i < length; i++)
    {
        if (ship_date_data[i] < 10592)
        {
            if (returnflag[i] == 100)
            {
                if (linestatus[i] == 100) { plain_filter_res_YY[i] = 1; plain_agg_res_YY += quantity[i]; }
                else                      { plain_filter_res_YN[i] = 1; plain_agg_res_YN += quantity[i]; }
            }
            else
            {
                if (linestatus[i] == 100) { plain_filter_res_NY[i] = 1; plain_agg_res_NY += quantity[i]; }
                else                      { plain_filter_res_NN[i] = 1; plain_agg_res_NN += quantity[i]; }
            }
        }
    }
    std::cout << "Data generation done." << std::endl;

    // ===================== START CLOCK (key setup) =====================
    ...
    auto qty_ciphers = cc->Encrypt(keyPair.publicKey, plain_qty);

    Plaintext plain_linestatus = cc->MakeCKKSPackedPlaintext(linestatus);
    Plaintext plain_linestatus_Y = cc->MakeCKKSPackedPlaintext(linestatus_predicate_Y);
    Plaintext plain_linestatus_N = cc->MakeCKKSPackedPlaintext(linestatus_predicate_N);
    Plaintext plain_returnflag = cc->MakeCKKSPackedPlaintext(returnflag);
    Plaintext plain_returnflag_Y = cc->MakeCKKSPackedPlaintext(returnflag_predicate_Y);
    Plaintext plain_returnflag_N = cc->MakeCKKSPackedPlaintext(returnflag_predicate_N);

    auto returnflag_ciphers = cc->Encrypt(keyPair.publicKey, plain_returnflag);
    auto linestatus_ciphers = cc->Encrypt(keyPair.publicKey, plain_linestatus);
    auto returnflag_predicate_Y_ciphers = cc->Encrypt(keyPair.publicKey, plain_linestatus_Y);
    auto linestatus_predicate_Y_ciphers = cc->Encrypt(keyPair.publicKey, plain_returnflag_Y);
    auto linestatus_predicate_N_ciphers = cc->Encrypt(keyPair.publicKey, plain_returnflag_N);

    // --- Filtering ---
    auto filter_start = std::chrono::system_clock::now();

    Ciphertext<lbcrypto::DCRTPoly> comp_res_ge, comp_returnflag_YY, comp_returnflag_YN, comp_linestatus_YY, comp_linestatus_YN;
    ...
    double filter_ms = std::chrono::duration_cast<std::chrono::milliseconds>(filter_end - filter_start).count();
    printf("filter time (actual) = %.0f ms\n", filter_ms);
    std::cout << "Filtering finish" << std::endl;

    // --- Aggregation ---
    std::cout << "Aggregation: SUM(quantity)" << std::endl;
    auto agg_start = std::chrono::system_clock::now();
    ...
    for (size_t i = 0; i < logrow; i++)
    {
        ...
        temp = sum_qty_cipher_YY; rot_temp = cc->EvalRotate(temp, step); sum_qty_cipher_YY = cc->EvalAdd(sum_qty_cipher_YY, rot_temp);
        temp = sum_qty_cipher_YN; rot_temp = cc->EvalRotate(temp, step); sum_qty_cipher_YN = cc->EvalAdd(sum_qty_cipher_YN, rot_temp);
        temp = sum_qty_cipher_NY; rot_temp = cc->EvalRotate(temp, step); sum_qty_cipher_NY = cc->EvalAdd(sum_qty_cipher_NY, rot_temp);
        temp = sum_qty_cipher_NN; rot_temp = cc->EvalRotate(temp, step); sum_qty_cipher_NN = cc->EvalAdd(sum_qty_cipher_NN, rot_temp);
    }

    auto agg_end = std::chrono::system_clock::now();
    double agg_ms = std::chrono::duration_cast<std::chrono::milliseconds>(agg_end - agg_start).count();
    printf("agg time (actual) = %.0f ms\n", agg_ms);
    ...
    // ===================== STOP CLOCK =====================
    auto e2e_end = std::chrono::system_clock::now();
    ...
    std::cout << "Query Evaluation Time (actual): " << e2e_ms << " ms" << std::endl;
    std::cout << "  key setup + encrypt + filter + agg + decrypt" << std::endl;
    std::cout << "--------------------------------------------------------" << std::endl;
...
double Eval_E2E_q12(int records_num)
{
    ...
    std::vector<double> shipdate(length), commitdate(length), receiptdate(length), orderpriority(length), shipmode(length),
        predicate_date1(length, 10000), predicate_date2(length, 13000),
        predicate_mail(length, 10), predicate_ship(length, 20),
        predicate_urgent(length, 10), predicate_high(length, 20);
    ...
    for (size_t i = 0; i < length; i++)
    {
        bool ress = (commitdate[i] < receiptdate[i] && shipdate[i] < commitdate[i] &&
        ...
        if (ress && shipmode[i] == 10)
        {
            ...
            agg_mail_res += 1;
            if (plain_filter_order[i] == 1) { plain_res_mail_order[i] = 1; agg_mail_order_res += 1; }
        }
        if (ress && shipmode[i] == 20)
        {
            plain_filter_res_ship[i] = 1;
            agg_ship_res += 1;
            if (plain_filter_order[i] == 1) { plain_res_ship_order[i] = 1; agg_ship_order_res += 1; }
        }
    }
    std::cout << "Data generation done." << std::endl;
    ...
    Plaintext plain_shipmode = cc->MakeCKKSPackedPlaintext(shipmode);
    auto shipmode_ciphers = cc->Encrypt(keyPair.publicKey, plain_shipmode);
    Plaintext plain_orderpriority = cc->MakeCKKSPackedPlaintext(orderpriority);
    auto orderpriority_ciphers = cc->Encrypt(keyPair.publicKey, plain_orderpriority);
    Plaintext plain_predicate_mail = cc->MakeCKKSPackedPlaintext(predicate_mail);
    auto predicate_mail_cipher = cc->Encrypt(keyPair.publicKey, plain_predicate_mail);
    Plaintext plain_predicate_ship = cc->MakeCKKSPackedPlaintext(predicate_ship);
    auto predicate_ship_cipher = cc->Encrypt(keyPair.publicKey, plain_predicate_ship);
    Plaintext plain_predicate_urgent = cc->MakeCKKSPackedPlaintext(predicate_urgent);
    auto predicate_urgent_cipher = cc->Encrypt(keyPair.publicKey, plain_predicate_urgent);
    Plaintext plain_predicate_high = cc->MakeCKKSPackedPlaintext(predicate_high);
    auto predicate_high_cipher = cc->Encrypt(keyPair.publicKey, plain_predicate_high);

    // --- Filtering + Aggregation ---
    auto filter_start = std::chrono::system_clock::now();

    Ciphertext<lbcrypto::DCRTPoly> filter_res_mail, filter_res_ship, order_res, pre_res;
    Ciphertext<lbcrypto::DCRTPoly> count_mail, count_ship, count_mail_order, count_ship_order;
    Ciphertext<lbcrypto::DCRTPoly> agg_mail, agg_ship, agg_mail_order, agg_ship_order;
    ...
    {
        int step = 1 << (logrow - i - 1);
        temp = agg_mail; rot_temp = cc->EvalRotate(temp, step); agg_mail = cc->EvalAdd(agg_mail, rot_temp);
        temp = agg_ship; rot_temp = cc->EvalRotate(temp, step); agg_ship = cc->EvalAdd(agg_ship, rot_temp);
        temp = agg_mail_order; rot_temp = cc->EvalRotate(temp, step); agg_mail_order = cc->EvalAdd(agg_mail_order, rot_temp);
        temp = agg_ship_order; rot_temp = cc->EvalRotate(temp, step); agg_ship_order = cc->EvalAdd(agg_ship_order, rot_temp);
    }

    auto filter_end = std::chrono::system_clock::now();
    double filter_agg_ms = std::chrono::duration_cast<std::chrono::milliseconds>(filter_end - filter_start).count();
    printf("filter+agg time (actual) = %.0f ms\n", filter_agg_ms);
...
double Eval_E2E_q6(int records_num)
{
    ...
    std::vector<double> revenue(length), ship_date(length), discount(length), quantity(length),
        predicate1_value(length, 10592), predicate2_value(length, 10957),
        predicate3_value(length, 3500), predicate4_value(length, 4500), predicate5_value(length, 2450);
    ...
    std::vector<Ciphertext<lbcrypto::DCRTPoly>> shipdate_ciphers, discount_ciphers, quantity_ciphers,
        predicate_value_cipher1, predicate_value_cipher2, predicate_value_cipher3,
        predicate_value_cipher4, predicate_value_cipher5;
    ...
    Plaintext plain_revenue = cc->MakeCKKSPackedPlaintext(revenue);
